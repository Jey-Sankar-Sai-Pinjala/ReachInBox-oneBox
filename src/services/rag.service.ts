...
import { qdrantClient, upsertVector, searchSimilar } from '../config/qdrant';
import { fetchGemini } from '../utils/fetchHelper';
...
export class RAGService {
  ...
  public async seedProductData(): Promise<void> {
    const productData = [
      {
        id: uuidv4(),
    ...
    logger.info('🌱 Seeding product data into vector database...');
    ...
    for (const item of productData) {
      try {
        ...
        await upsertVector(item.id, embedding, {
          content: item.content,
          type: item.type,
          createdAt: new Date().toISOString()
        });

        logger.info(`✅ Seeded product data: ${item.id}`);
      } catch (error) {
        logger.error(`❌ Error seeding product data ${item.id}:`, error);
      }
    }

    logger.info('✅ Product data seeding completed');
  }
  ...
  public async suggestReply(email: EmailDocument): Promise<string> {
    try {
      ...
      const relevantContexts = await searchSimilar(emailEmbedding, 3);
      
      logger.info(`🔍 Found ${relevantContexts.length} relevant contexts`);
      
      if (relevantContexts.length === 0) {
        logger.warn('⚠️  No relevant context found for reply suggestion');
        return this.generateGenericReply(email);
      }

      // Step 3: Build context from retrieved documents
      const contextText = relevantContexts
        .map((ctx: any) => ctx.payload.content)
        .join('\n\n');

      logger.info(`📝 Context: ${contextText.substring(0, 200)}...`);

      // Step 4: Generate reply using RAG (with fallback)
      try {
        const reply = await this.generateRAGReply(email, contextText);
        logger.info(`✅ RAG reply suggestion generated for email: ${email.id}`);
        return reply;
      } catch (error) {
        logger.warn('⚠️  RAG generation failed, using contextual reply:', error);
  ...
  private async generateRAGReply(email: EmailDocument, context: string): Promise<string> {
    const prompt = `
...
`;
  ...
  private generateGenericReply(email: EmailDocument): string {
...
[Your Name]`;
  ...
  private generateContextualReply(email: EmailDocument, context: string): string {
    // Simple rule-based reply generation using context
    const subject = email.subject.toLowerCase();
    const body = email.body.toLowerCase();
    
    // Check for meeting/interview related keywords
...
[Your Name]`;
    ...
    // Check for job/position related keywords
...
[Your Name]`;
  ...
  public async addCustomContext(content: string, type: string): Promise<void> {
    try {
      ...
      logger.info(`✅ Custom context added: ${id}`);
    } catch (error) {
      logger.error('❌ Error adding custom context:', error);
      throw error;
    }
  }

  public async searchContext(query: string, limit: number = 5): Promise<any[]> {
    try {
      const queryEmbedding = await this.generateEmbedding(query);
      return await searchSimilar(queryEmbedding, limit);
    } catch (error) {
  ...
  public async getContextStats(): Promise<{
    ...
    // This would typically query your vector database
  ...
  public async updateContext(id: string, content: string): Promise<void> {
    try {
      const embedding = await this.generateEmbedding(content);
  ...
  public async deleteContext(id: string): Promise<void> {
    try {
      // Implementation depends on your vector database client
      // For Qdrant, you would use: await qdrantClient.delete(config.qdrant.collection, { points: [id] });
